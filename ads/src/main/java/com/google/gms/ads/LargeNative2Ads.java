...
public class LargeNative2Ads {
    ...
    public void loadNativeAds(Activity activity) {
        final String nativeAdstr = PowerPreference.getDefaultFile().getString(AdUtils.NATIVE2ID, "123");
    ...
    public void loadNativeAds2(Activity activity) {
        final String nativeAdstr = PowerPreference.getDefaultFile().getString(AdUtils.NATIVE2ID2, "123");
    ...
    public void showNativeAds(Activity activity, LinearLayout adLayout, int ad_type) {
        ...
        if (gNativeAd.size() > 0) {
            ...
            showNativeAds1(activity, adLayout, ad_type);
        }
    }

    public void showNativeAds1(Activity activity, LinearLayout adLayout, int ad_type) {
        final String nativeAdstr = PowerPreference.getDefaultFile().getString(AdUtils.NATIVE2ID, "123");
        if (!nativeAdstr.equalsIgnoreCase("--")) {
            AdLoader adLoader = new AdLoader.Builder(activity, nativeAdstr).forNativeAd(new NativeAd.OnNativeAdLoadedListener() {
                ...
                public void onAdFailedToLoad(LoadAdError errorCode) {
                    AdUtils.showLog(errorCode.toString());
                    showNativeAds2(activity, adLayout, ad_type);
                }
            }).withNativeAdOptions(new NativeAdOptions.Builder()
                    .setVideoOptions(new VideoOptions.Builder()
                            .setStartMuted(true)
    ...
    public void showNativeAds2(Activity activity, LinearLayout adLayout, int ad_type) {
